$(document).ready(function(){
	...
	$("#slider-form").submit(function(e){
	    ...
	    if(inputFile.val()!=""){
	    	...
	    	formData.append("company_name", comp_name);
	    	...
	    	$.ajax({
	    		url: App.apiUrl+'/admin/advertisement',
	    		type: 'POST',
	    		data: formData,
	    		processData: false,
	    		contentType: false,
	...
	$("input[name = userfile]").on("change", function(){ 
 		...
 		image.onload = function(){  
 			...
 			if((this.naturalWidth < 1300) && (this.naturalHeight < 400))
 			{
 				...
 				$.notify({
 					title: "",
 					message: "<i class='fa fa-exclamation-triangle'></i> &nbsp&nbspImage Dimension is at least 1300 PIXELS to 400 PIXELS",
 					target: "_blank"
 				},{
 					type: "danger",
 					animate: {
 	...
 	var slider_table = $('#slider-list-table').DataTable({ 
           	// "order": [[ 4, "desc" ]],
            "processing": true, 
            "serverSide": true, 
            "responsive": true,
            ...
            "ajax": {
                "url": App.pathUrl + "/admin/advertisements/sliders",
                "type": "POST",
 					...
 					buttons: {
 						confirm: {
 							...
 							action: function () {
 								...
 								$.ajax({
 									type: 'POST',
 									url: App.apiUrl+'/admin/advertisement/activate',
 									dataType: 'JSON',
 									data:{
 										id : slid_id,
 									},
 					...
 					buttons: {
 						confirm: {
 							...
 							action: function () {
 								...
 								$.ajax({
 									type: 'POST',
 									url: App.apiUrl+'/admin/advertisement/deactivate',
 									dataType: 'JSON',
 									data:{
 										id : slid_id,
 									},
 	...
 	$(document).on('shown.bs.modal', '#dynamicModal', function (evt) {
        ...
        var data = getJSONDoc(App.apiUrl + "/admin/advertisement/id/"+slid_id);
       	...
       	if(btn_id == "btn-edit")
       	{
	        ...
	        $(this).find('.modal-title').text("Edit Advertisement");
       	...
       	if(btn_id == "btn-view")
       	{
       		html += '<div class="" style="max-height: 400px; overflow: hidden;">';
    ...
    $(document).on("click", "#update-ads", function(){
    	var comp_name = $.trim($("#edit-ads input[name=company_name]").val());
